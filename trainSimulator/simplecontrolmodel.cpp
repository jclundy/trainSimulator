...
SimpleControlModel::SimpleControlModel(float maxSpeed, float maxAcceleration, float maxBraking, float errorThreshold)
{
    m_maxSpeed = maxSpeed;
...
void SimpleControlModel::computeNewStatesBraking(float &speed, float &acceleration, float speedSetpoint, float dt) {
    // if speed is greater than max speed, slow it down
    if(speedSetpoint > m_maxSpeed) {
        speedSetpoint = m_maxSpeed;
    }
